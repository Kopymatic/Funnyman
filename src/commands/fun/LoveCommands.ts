     ...
     */
     ...
     override async run(msg: Message<TextableChannel>): Promise<string> {
         if(msg.mentions.length > 0) {
            ...
            let userName: string;
            if(user.nick == null) { userName = user.username } else {userName = user.nick}
            let mentionName: string;
            if(mention.nick == null) { mentionName = mention.username } else {mentionName = mention.nick}

            msg.channel.createMessage({embed:{
                title: `${userName} ${this.embedTitleText} ${mentionName}`,
                image: {url: this.gifs[randomInt(this.gifs.length)]},
                footer: {text: `That's ${result.timesperformed} ${this.embedFooterText} now!`},
